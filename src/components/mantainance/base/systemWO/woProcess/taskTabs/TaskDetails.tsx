      ...
      <div className="flex p-3 flex-col mt-2 mx-auto border-8  overflow-hidden ">
        ...
        >
          ...
          <Descriptions.Item label="Zones">
            <List>
              {task?.zonesArr &&
                task?.zonesArr.map((item: any, index: any) => (
                  <List.Item key={index}>
                    <Space>
                      <div>Zone: {item.zoneNbr}</div>
                      <div>
                        {item.majoreZone} {item.subZone}
                      </div>
                    </Space>
                  </List.Item>
                ))}
          ...
          <Descriptions.Item label="Access">
            <List>
              {task?.removeInslallItemsIds &&
                task?.removeInslallItemsIds.map((item, index) => (
                  <List.Item key={index}>
                    <Space>
                      <div>Panel: {item.accessItemID.panel}</div>
                      <div>Zone: {item.accessItemID.majoreZone}</div>
                      <div>Sub Zone: {item.accessItemID.subZone}</div>
                      <div>
                        Status:{" "}
