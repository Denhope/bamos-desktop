...
const GroupTaskList: FC<IGroupTaskListPrors> = ({
  ...
  const fetch = () => {};
  const { isLoading, currentProject, projectGroups, projectTasks } =
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (companyID) {
        ...
        setProjectGroupTasks(dataSource);
      }
    };
    fetchData();
  ...
  const onChange: any["props"]["onChange"] = async (
    ...
    if (result.meta.requestStatus === "fulfilled") {
      ...
      selectedRowKeys.forEach((rowKey) => {
        const index = projectGroupTasks.findIndex(
  ...
  const initialColumns: ProColumns<any>[] = [
      ...
      ...useColumnSearchProps({
        ...
        onSearch: (value) => {
          if (value) {
            ...
            // Отобразите все данные, если поисковый запрос пуст
            setDataSource(projectGroupTasks);
          }
        },
        data: dataSource,
      }),
          ...
          Array.isArray(dataSource)
            ? dataSource.flatMap((item) =>
                Array.isArray(item.zonesArr)
                  ? item.zonesArr.map(
                      (zone: { majoreZoneShort: string }) =>
                        zone.majoreZoneShort
                    )
                  : []
              )
            : []
    ...
    },
    {
        ...
        new Set(
          Array.isArray(dataSource)
            ? dataSource.flatMap((item) =>
                Array.isArray(item.zonesArr)
                  ? item.zonesArr.map(
                      (zone: { subZone: string }) => zone.subZone
                    )
                  : []
              )
            : []
      ...
      //     dataSource.flatMap((item) =>
      //       Array.isArray(item.zonesArr)
      //         ? item.zonesArr.map((zone: { subZone: string }) => zone.subZone)
      //         : []
      //     )
      //   )
      // ).map((value) => ({ text: value, value })),
      onFilter: (value, record) =>
        record.zonesArr.some(
          (zone: { subZone: string }) => zone.subZone === value
        ),
    },
    {
      title: `${t("ZONE ID")}`,
