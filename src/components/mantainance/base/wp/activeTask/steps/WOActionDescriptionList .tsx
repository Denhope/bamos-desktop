...
const WOActionDescriptionList: FC<IActionDescriptionListPrors> = ({ data }) => {
  ...
  useEffect(() => {
    if (currentProjectTask) {
      dispatch(setCurrentAction(currentProjectTask?.actions?.[0] || null));
      setSelectedRowKey(1);
    }
  }, []);
  ...
  useEffect(() => {
    if (currentActiveIndex) {
      dispatch(
        setCurrentAction(
          currentProjectTask?.actions?.[currentActiveIndex] || null
        )
      );
    }
  }, [currentAction, currentActiveIndex, currentProjectTask]);
  useEffect(() => {
    if (currentProjectTask) {
      setEditableRecord(
  ...
  const handleDeleteSelectedItems = () => {
    ...
    const updatedData = data.filter(
      (row) => row.actionNumber && !selectedRowKeys.includes(row.actionNumber)
    );
