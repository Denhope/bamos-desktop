...
export function PickSlipStatus() {
  ...
  // Обновляем useMemo для issuedPickSlips с фильтрацией по времени
  const issuedPickSlips = useMemo(() => {
    const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);

    return pickSlips.filter((slip) => {
  ...
  const renderContent = () => {
    if (isCardView) {
      ...
      // Комбинируем все отфильтрованные массивы для таблицы
      const filteredPickSlips = [
        ...inProgressPickSlips,
        ...completedPickSlips,
        ...issuedPickSlips,
        ...toFixedPickSlips,
      ...
      return (
        <div style={{ height: gridHeight, width: '100%' }}>
          <AgGridReact<PickSlip>
            ref={gridRef}
            columnDefs={columns}
            rowData={sortedPickSlips} // Используем отфильтрованные данные
