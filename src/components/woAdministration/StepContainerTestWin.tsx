...
interface Props {
  ...
  templates: Template[];
  task?: any;
  // Update the type of templates to Template[]
}
interface Template {
  description: any;
  id: string;
...
const StepContainer: React.FC<Props> = ({
  ...
  const { openWindow } = useElectronWindow();
    ...
    <div className="py-3">
      {steps.length > 0 ? (
        <div>
          ...
          {[...steps] // Создаем копию массива с помощью spread оператора
            .sort((a, b) => {
              // Преобразуем даты в timestamp для сравнения
              const dateA = new Date(a.createDate).getTime();
              const dateB = new Date(b.createDate).getTime();
              return dateA - dateB; // Сортировка по возрастанию (старые -> новые)
            })
            .map(
              (step, index) => (
                console.log(index),
