    ...
    if (isPdfRequested && !isLoading && projectTasks) {
      const generatePdfDocument = async () => {
        try {
          ...
          const viewFileName = `TASK-${firstTask?.taskNumber || 'unknown'}`;
          ...
          if (pdfAction === 'view') {
            ...
            // Создаем временную ссылку для открытия с правильным именем файла
            const tempLink = document.createElement('a');
            tempLink.href = fileURL;
            tempLink.target = '_blank';
            tempLink.download = `${viewFileName}.pdf`; // Это установит правильное имя файла

            // Открываем PDF в новом окне
            const newWindow = window.open(fileURL, '_blank');
            if (newWindow) {
              newWindow.document.title = `${viewFileName}.pdf`;
            }
            ...
            link.href = fileURL;
            link.download = `${viewFileName}-${formattedDate}.pdf`;
