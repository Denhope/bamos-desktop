...
const PdfGenerator: React.FC<{
  ...
  const {
    ...
    {
      skip: !ids.length,
      refetchOnMountOrArgChange: true, // Добавьте этот параметр
      refetchOnFocus: true,
    }
  );
  ...
  const generateAndSavePdf = async () => {
    ...
    try {
      ...
      // Получаем первую задачу из массива transformedTasks
      ...
      link.href = fileURL;
      link.download = `TASK-${
        firstTask?.taskNumber || 'unknown'
      }-${formattedDate}.pdf`; // Добавляем fallback
