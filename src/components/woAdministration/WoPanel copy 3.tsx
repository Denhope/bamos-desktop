    ...
    }
    {
      actionType == 'reOpen' &&
        Modal.confirm({
          ...
          onOk: async () => {
            try {
              const currentEditingProject = { ...editingProject }; // Сохраняем текущий editingProject
              await addMultiAction({ actionType, ids });
              await refetch(); // Дожидаемся обновления данных

              setEditingProject(null);
              // setSelectedRowId(null);

              // Находим обновленную версию задачи в обновленных данных
              const updatedProject = transformedTasks.find(
                (task) => task.id === currentEditingProject.id
              );
              ...
              if (updatedProject) {
                ...
                // Если задача не найдена, возможно, она была удалена или изменена
                setSelectedRowId(ids[0]); // Выбираем первую задачу из списка reopened
              }
