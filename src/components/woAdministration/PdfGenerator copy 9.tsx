...
const PdfGenerator: React.FC<{
          ...
          if (actions.length > 0) {
            ...
            actions.forEach((action, index) => {
              ...
              if (y - (actionDescriptionHeight + 50) < 50) {
                ...
                drawFooter(page);
                ({ page, width, height, y } = addPage());
                y = height - 50; // Устанавливаем y в верхнюю часть новой страницы
              }

              const actionY = y - actionDescriptionHeight - 35;
              ...
              if (action.type === 'pfmd') {
                page.drawText(`Action-step ${i + 1}-${index + 1}`, {
                  x: cellX + 5,
                ...
                page.drawText(`Inspection-step ${i + 1}-${index + 1}`, {
                  x: cellX + 5,
                  y: actionY + actionDescriptionHeight + 40,
                  font: robotoFont,
                  size: fontSize,
  ...
  const drawActions = (page: PDFPage, actions: ActionType[], y: number) => {
    const actionHeight = 30; // Высота одной строки акшена
    const spaceThreshold = 70; // Минимальное пространство для новой страницы
    const pageHeight = 841.89; // Высота A4 в пунктах

    for (const action of actions) {
      console.log(`Drawing action: ${action.description}, Current y: ${y}`); // Логирование

      // Проверяем, достаточно ли места для следующего акшена
      if (y - actionHeight < spaceThreshold) {
        console.log('Adding new page'); // Логирование
        page = pdfDoc.addPage([595.28, pageHeight]); // A4 размер
        y = pageHeight - 30; // Сброс y на новую страницу
      }

      // Рисуем акшен
      page.drawText(action.description, {
        x: 50,
        y: y,
        font: robotoFont,
        size: 12,
      });
      y -= actionHeight; // Обновляем y для следующего акшена
    }

    return y; // Возвращаем обновленный y
  };
  ...
  const generatePdfDocument = async () => {
    if (isPdfRequested && !isLoading && projectTasks) {
      ...
      y = drawActions(page, actions, y); // Обновляем y после рисования акшенов
