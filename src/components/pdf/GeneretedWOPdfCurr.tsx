...
const GeneretedWOPdf: FC<GeneretedWOPdfProps> = ({ task, scroll }) => {
  ...
  const QRCodeImg = document.getElementById(
    `qrGenerator-${currentProjectTask?.projectTaskWO}`
  ) as HTMLCanvasElement;
  const QRCodeDataURL = QRCodeImg?.toDataURL() || "";
  const dispatch = useAppDispatch();
  dispatch(setCurrentQrCodeLink(QRCodeDataURL));
  ...
  return (
    <div className="">
      <QRGenerator
        valueString={String(
          String(
            (currentProjectTask && currentProjectTask?.projectTaskWO) || ""
          )
        )}
