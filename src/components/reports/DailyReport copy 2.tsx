...
const DailyReport: FC<DailyReportProps> = ({ workOrder, tasks, accesses }) => {
  ...
  const exportToPDF = () => {
    ...
    // Time Statistics by Task Type
    doc.text('Time Statistics by Task Type', 14, doc.lastAutoTable.finalY + 10);
    ...
    doc.autoTable({
      startY: doc.lastAutoTable.finalY + 15,
      head: [['Task Type', 'Planned Time (hours)', 'Actual Time (hours)']],
      body: taskTypeTimeData,
      foot: [
        [
          'Total',
          Object.values(timeByTaskType)
            .reduce((sum, times) => sum + times.planned, 0)
            .toFixed(2),
          Object.values(timeByTaskType)
            .reduce((sum, times) => sum + times.actual, 0)
            .toFixed(2),
        ],
      ],
    });
    ...
    doc.autoTable({
      ...
      foot: [
        [
          'Total',
          Object.values(calculateTimeBySkill)
    ...
    doc.text('Summary Time Statistics', 14, doc.lastAutoTable.finalY + 10);
    doc.autoTable({
      startY: doc.lastAutoTable.finalY + 15,
      head: [['Category', 'Time (hours)']],
      body: [
        ['Planned Time', timeStats.plannedTime.toFixed(2)],
        ['Routine Tasks Time', timeStats.spentTime.toFixed(2)],
        ['NRC Time', timeStats.defectTime.toFixed(2)],
        [
          'Total Actual Time',
          (timeStats.spentTime + timeStats.defectTime).toFixed(2),
        ],
      ],
    });
