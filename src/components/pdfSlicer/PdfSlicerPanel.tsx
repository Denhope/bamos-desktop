...
import { Upload, Button, Input, Spin, Space, Row, Col, Divider, List, Tabs, Modal, Tooltip, Statistic, Card, notification } from 'antd';
...
const PdfSlicerPanel: React.FC<PdfSlicerPanelProps> = ({ onSavePages }) => {
  ...
  const openSaveDirectory = useCallback(() => {
    if (saveDirectory && window.electronAPI) {
      ...
      notification.error({
        message: 'Error',
        description: 'Save directory is not set or Electron API is not available',
      });
    }
  }, [saveDirectory]);
    ...
    if (files) {
      ...
      if (validFiles.length !== files.length) {
        notification.warning({
          message: 'Warning',
          description: 'Some files were filtered out as they do not meet the requirements.',
        });
      }
  ...
  const renderUploadTab = () => {
    ...
    return (
      <Space direction="vertical" style={{ width: '100%', height: '100%' }}>
        <Button icon={<InfoCircleOutlined />} onClick={() => notification.info({
          message: 'File Format Information',
          description: 'Files should be in PDF format and named as XXX_task_*.pdf, where XXX is a prefix of 3 or 4 letters',
        })}>
        ...
        {activeTab === '1' && pdfFile && (
          <>
            <div style={{ marginBottom: 16 }}>
              <p><strong>File name:</strong> {pdfFile.name}</p>
        ...
        {activeTab === '2' && (
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            ...
            {(uploadResults.success.length > 0 || uploadResults.error.length > 0) ? (
              ...
              <Card>
                <p>File upload has not been performed yet or there are no results to display.</p>
              </Card>
            )}
          </div>
        )}
