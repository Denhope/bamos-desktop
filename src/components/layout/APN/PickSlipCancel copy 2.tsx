...
const PickSlipCancel: FC<PickSlipCancelProps> = ({ onFinishCancel }) => {
  ...
  const handleSubmitCancel = () => {
    const isAnyRowInvalid = issuedData.some((row) => !isRowValid(row));
    if (isAnyRowInvalid) {
      notification.error({
        message: t('ERROR'),
        description: t(
          'Please fill in all required fields (QTY CANCEL and LOCATION TO) for all rows.'
        ),
      });
      return;
    }
  ...
  const onFinish = async (values: any) => {
    ...
    const invalidRows = issuedData.filter((item) => !isRowValid(item));
    if (invalidRows.length > 0) {
      notification.error({
        message: t('ERROR'),
        description: t(
          'Please fill in QTY CANCEL and LOCATION TO for all items.'
        ),
      });
      return;
    }

    const updatedItems = issuedData.map((item) => {
    ...
    try {
      ...
      // Обновите данные после успешной отмены
      refetchItems();
      refetch();
    } catch (error) {
      notification.error({
