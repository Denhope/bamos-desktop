            ...
            <Space align="center">
              ...
              <Col>
                ...
                <PDFExport
                  ...
                  filename={`pickslip_report_${
                    pickSlips && pickSlips[0]?.pickSlipNumberNew
                  }`}
                  headerInfo={{
                    'Pick Slip Number':
                      (pickSlips && pickSlips[0]?.pickSlipNumberNew) || '-',
                    Store:
                      (pickSlips && pickSlips[0]?.getFromID?.storeShortName) ||
                      '-',
                    WO:
                      (pickSlips &&
                        pickSlips[0]?.projectID?.WOReferenceID?.WONumber) ||
                      '-',
                    WP:
                      (pickSlips && pickSlips[0]?.projectID?.projectName) ||
                      '-',
                    'Trace No':
                      (pickSlips && pickSlips[0]?.projectTaskID?.taskWO) || '-',
                    'Task No':
                      (pickSlips && pickSlips[0]?.projectTaskID?.taskNumber) ||
                      '-',
                    'Task type':
                      (pickSlips &&
                        pickSlips[0]?.projectTaskID?.projectItemType) ||
                      '-',
