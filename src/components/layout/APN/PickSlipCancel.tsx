...
const PickSlipCancel: FC<PickSlipCancelProps> = ({ onFinishCancel }) => {
  ...
  const columnBookedDefs: ColDef[] = [
    {
      headerName: t('LABEL'),
      field: 'storeItemID.LOCAL_ID',
      editable: false,
      cellDataType: 'text',
      valueGetter: (params: any) => {
        return params.data?.storeItemID?.LOCAL_ID || '';
      },
    },
    {
  ...
  const getReturnSlipData = useCallback(() => {
    ...
    // Группируем отмененные позиции по дате отмены и пользователям
    const cancelGroups = new Map<string, any[]>();

    pickSlipItems.forEach((item) => {
      if (!item.bookedItems?.[0]?.canceledItems?.length) return;

      item.bookedItems[0].canceledItems.forEach((canceledItem) => {
        const storeItem = item.bookedItems?.[0]?.storeItemID || {};
        // Создаем ключ группировки из даты и ID пользователей
        const groupKey = `${canceledItem.cancelDate}_${canceledItem.cancelUserID?.id}_${canceledItem.storeManID?.id}`;

        if (!cancelGroups.has(groupKey)) {
          console.log(canceledItem);
          cancelGroups.set(groupKey, {
            items: [],
            cancelDate: canceledItem.cancelDate,
            storeMan: canceledItem.storeManID
              ? `${canceledItem.storeManID.firstNameEnglish} ${canceledItem.storeManID.lastNameEnglish}`
              : '',
            mechanic: canceledItem.cancelUserID
              ? `${canceledItem.cancelUserID.firstNameEnglish} ${canceledItem.cancelUserID.lastNameEnglish}`
              : '',
          });
        }

        const group = cancelGroups.get(groupKey);
        group.items.push({
          PART_NUMBER_BOOKED: storeItem.PART_NUMBER || '',
          DESCRIPTION: storeItem.NAME_OF_MATERIAL || '',
          QTYCANCEL: canceledItem.quantity || 0,
          LOCATION: storeItem?.locationID?.locationName || 'RESERVATION',
          LOCATION_TO: canceledItem?.locationID?.locationName || 'RESERVATION',
          LOCAL_ID: storeItem.LOCAL_ID || '',
        });
      });
    });

    if (cancelGroups.size === 0) return null;
    ...
    return {
      ...
      cancelGroups: Array.from(cancelGroups.values()),
    };
  }, [selectPickSlip, pickSlipItems]);

  const onFinish = async (values: any) => {
    if (!values.storeManID || !values.cancelUserID || !values.bookingDate) {
      notification.error({
        message: t('ERROR'),
