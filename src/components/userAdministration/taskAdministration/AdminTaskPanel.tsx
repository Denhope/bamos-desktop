...
// Обновляем enum для режимов импорта
...
const AdminTaskPanel: React.FC<AdminPanelProps> = ({ values, isLoadingF }) => {
  ...
  // Созда таблицу специальностей на основе полуенных данны
  ...
  const getZonesConfig = () => {
    ...
    const groupedZones = zones?.reduce((acc: any[], zone) => {
      ...
      if (existingZone) {
        // Если такая зона уже есть, оаляем только area информацию
  ...
  // Добавляем функцию для конвертции JOB_ZONE в коды зон
  ...
  const getTextInstructionsConfig = (taskType: string) => ({
    ...
    content: {
      SMC: [
        ...
        '5. Проверьте анные в предпросмотре',
  ...
  // Функция для проверки существования партномра
  const validatePartNumber = (partNumber: string) => {
    ...
    const foundPart = partNumbers?.find(
      (part) =>
        ...
        String(part.id) === String(partNumber)
    );
  ...
  const handleAddMaterials = async (data: any[]) => {
    ...
    try {
      if (importMode === 'single') {
        ...
        if (!taskExists) {
          throw new Error(t('SELECTED_TASK_NOT_FOUND'));
        }
      }
      ...
      const processedData = data.map((item, index) => {
        ...
        if (!foundPart) {
          throw new Error(`Invalid part number at row ${index + 2}`);
        }
        ...
        // Добавляем обработку isRequired
        const isRequired = Boolean(
          item.isRequred && item.isRequred.toString().toLowerCase() === 'true'
        );

        if (importMode === 'multiple') {
          const task = transformedTasks?.find(
            (t) => t.taskNumber === item.taskNumber
          );
          if (!task) {
            throw new Error(
              `Task not found: ${item.taskNumber} at row ${index + 2}`
            );
          }
          return {
      ...
      await addMultiPartTasks({ parts: processedData }).unwrap();
  ...
  const getMaterialsTemplateConfig = () => {
    const baseFields = [
      ...
      {
        name: 'isRequred',
        displayName: t('IS_REQUIRED'),
        required: true,
        description: t('IS_REQUIRED_DESCRIPTION'),
        example: 'true',
        width: 10,
        validation: (value: string) => {
          const strValue = String(value).toLowerCase();
          ...
          : [
              ...baseFields,
              {
                ...
                validation: (value: string) => {
                  ...
                  if (!taskExists) {
                    console.warn(`Task not found: ${value}`);
                  }
                  return taskExists;
                },
              },
            ],
      // Разные имена файлов для разных режимов
      templateFileName:
        importMode === 'single'
          ? 'materials_template_single_task.xlsx'
          : ('materials_template_multiple_tasks.xlsx' as const),
  ...
  }, [importMode, selectedTaskId, selectedAcType]);

  // Обновляем обработчик выбора задачи
          ...
          <Space className="gap-6">
            ...
            <Space direction="horizontal" size="small">
              <Switch
                ...
                defaultChecked
                onChange={(checked) => {
                  setImportMode(checked ? 'single' : 'multiple');
                  setSelectedTaskId(''); // Сбрасываем выбранный taskId при смене режима
                }}
              />
              <FileUploaderV2
                templateConfig={{
                ...
                disabled={importMode === 'single' && !selectedTaskId} // Блокируем кнопку если не выбрана задача
                additionalSelects={
                  importMode === 'single'
                    ? [
                        {
                          ...
                          onChange: (value: string) => {
                            console.log('Selected AC Type:', value);
                            setSelectedAcType(value);
                            setSelectedTaskId(''); // Сбрасываем taskId при смене типа ВС
                          },
                        },
                        {
                          key: 'taskId',
                          ...
                          width: 400,
                          dependsOn: 'acTypeId',
                          value: selectedTaskId, // Добавляем значение
                          onChange: handleTaskSelect, // Используем обновленный обработчик
                        },
                      ]
                    : []
                }
