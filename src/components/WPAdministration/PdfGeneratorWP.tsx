...
const PdfGeneratorWP: React.FC<{
  ...
  useEffect(() => {
    if (ids?.length && projectTasks && !isLoading && !isFetching) {
      setIsReady(true);
    } else {
      setIsReady(false);
    }
  }, [ids, projectTasks, isLoading, isFetching]);
  ...
  const generatePdf = async () => {
    // Добавляем определение formattedDate
    ...
    try {
      ...
      throw error; // Пробрасываем ошибку дальше для обработки в handleGeneratePdf
    } finally {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          notification.error({
            message: t('FAILED'),
            description: t('There was an error generating PDF.'),
          });
        } finally {
          setLoading(false);
        }
  ...
  return (
    <div className="ml-4">
      <Button
        ...
        loading={loading}
      >
        {loading ? t('GENERATING...') : t('GENERATE PDF AND SAVE')}
      </Button>
    </div>
  );
};
